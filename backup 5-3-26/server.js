...
- Add buyer-use terms when relevant: stickers, mugs, wall art, planner, shirts.
...
- Start by clearly saying what the buyer can make or use the file for.
...
function polishListing(data, originalTitle, vibe) {
  ...
  while (data.tags.length < 13) {
    data.tags.push("instant download");
    data.tags = [...new Set(data.tags)].slice(0, 13);
  }

  let improvedTitle = data.improvedTitle || "";

  if (type === "fruit" && !improvedTitle.toLowerCase().includes("fruit")) {
    improvedTitle = `Fruit Clipart Bundle | ${improvedTitle}`;
  }

  if (type === "animal" && !improvedTitle.toLowerCase().includes("animal")) {
    improvedTitle = `Animal Clipart Bundle | ${improvedTitle}`;
  }

  if (!improvedTitle.toLowerCase().includes("bundle")) {
  ...
  if (allText.includes("svg") && !improvedTitle.toLowerCase().includes("svg")) {
    improvedTitle += " | SVG";
  }

  if (allText.includes("png") && !improvedTitle.toLowerCase().includes("png")) {
    improvedTitle += " | PNG";
  }

  if (!improvedTitle.toLowerCase().includes("instant download")) {
...
function calculateScore(data) {
  ...
  if (title.includes("stickers") || title.includes("shirts") || title.includes("mugs") || title.includes("planner")) {
    breakdown.title += 2;
  }
...
function getMissedOpportunities(data) {
  ...
  return (map[type] || map.general)
    ...
    .map((keyword) => ({
      keyword,
      reason: "High-converting Etsy keyword not currently used.",
      impact: "This keyword can increase traffic and ranking if relevant to your product.",
    }));
}
...
function generateQuickWins(data) {
  ...
  if (!title.includes("instant download")) {
    wins.push("Include 'Instant Download' to capture ready-to-buy traffic.");
  }

  if (!description.includes("300 dpi")) {
    wins.push("Mention 300 DPI clearly to build buyer trust.");
  }
  ...
  if (!title.includes("stickers") && !title.includes("mugs") && !title.includes("wall art")) {
    wins.push("Add a clear use case like stickers, mugs, wall art, or planner designs.");
  }
